...
screen NOAA GHCN-Daily stations for completeness and quality;
delineate five climatic regions using Ward hierarchical clustering;
audit CMIP6 models for a complete common-member chain;
extract daily precipitation and temperature at station locations;
evaluate historical regional skill in five contiguous temporal blocks;
apply quantile-delta mapping independently to each model, station, month, and variable;
calculate twelve annual extreme indices separately for every model;
estimate model-specific future changes relative to each model's own baseline;
apply regional skill weights to model-level changes;
classify robust directional agreement among the six models;
assess residual historical bias and related-model sensitivity;
evaluate Mann–Kendall trends with Benjamini–Hochberg false-discovery-rate control; and
produce station-network-weighted regional summaries and publication products.
---
Quick Access
Resource	Repository location
Complete corrected notebook	`notebooks/Iowa_CMIP6_Corrected_Full_Pipeline.ipynb`
...
Validation and trend outputs	`outputs/validation_and_trends/`
Figure-source data	`outputs/figure_source_data/`
...
Open:
`notebooks/Iowa_CMIP6_Corrected_Full_Pipeline.ipynb`
Mount Google Drive when prompted.
update the project-root configuration cell;
run the notebook from top to bottom;
confirm that all validation checks pass; and
inspect the generated publication package.
